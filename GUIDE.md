...
- `<li>` - list item, элемент листа
- `<dl>` - definition list, список определений
- `<dt>` - definition term, определение
- `<dd>` - different definitions, другое определение
...
rel="noopener" = безопасно открыть в новой вкладке
rel="noopener noreferrer" = безопасно + не передавать referrer
...
```
<figure> — это контейнер для контента, который можно воспринимать как отдельную иллюстрацию / фигуру / пример.

Внутри <figure> часто помещают:
- изображение
- схему
- диаграмму
- график
...
<figure>      = самостоятельный блок с иллюстрацией / примером / таблицей
<figcaption>  = подпись к этому блоку
<img>         = изображение

Cовременный пример адаптивного изображения: браузер сам выбирает,
какую версию картинки загрузить маленькую или большую.
...
src      = основная картинка / запасной вариант (src = source = источник)
srcset   = список разных размеров картинки (srcset = source set = набор источников)
           images/photo-400.jpg 400w = файл photo-400.jpg имеет ширину 400 пикселей
           images/photo-800.jpg 800w = файл photo-800.jpg имеет ширину 800 пикселей
sizes    = какую ширину картинка будет занимать на экране
           sizes="(max-width: 600px) 400px, 800px" = Сколько места изображение будет занимать в макете.
           если экран маленький — картинка занимает 400px.
           если экран больше 600px — картинка занимает 800px.
alt      = текстовое описание изображения (alt = alternative text = альтернативный текст)
loading  = способ загрузки изображения, управляет тем, когда браузер загружает изображение
           loading="lazy" = загружать изображение позже, когда пользователь приблизится к нему при прокрутке.
           loading="eager" = загружать изображение сразу.
width    =
height   =
align    = выравнивание "left", "right", "top", "middle", "bottom".

<div> = это универсальный блочный контейнер в HTML, коробка / блок / контейнер для других элементов
        используют, когда нужно сгруппировать несколько элементов вместе.
        Сам по себе <div> визуально почти никак не выглядит.
<div>
  <h2>Игровое кресло Aerocool</h2>
  <p>Удобное кресло для работы, учебы и игр.</p>
  <a href="/products/">Смотреть товары</a>
</div>

Здесь <div> объединяет:
<h2> — заголовок
<p>  — текст
<a>  — ссылку

Но <div> очень часто используют вместе с CSS:

<div class="card">
  <h2>Aerocool Chair</h2>
  <p>Comfortable ergonomic chair.</p>
</div>
...
- `<tr>` - строка (table row)
- `<th>` - ячейка-заголовок (table heading)
- `<td>` - обычная ячейка (table data)
- `<thead>`, `<tbody>`, `<tfoot>` - смысловые группы строк.
- `colspan` - объединить столбцы. (colspan=2)
- `rowspan` - объединить строки. (rowspan=2)
